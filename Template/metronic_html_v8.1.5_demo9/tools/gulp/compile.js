                ...
                if (["custom", "media", "api", "misc"].indexOf(key) !== -1) {
                    if (
                        ...
                        for (let i in val.styles) {
                            ...
                            if (
                                toRtlFile.indexOf(".scss") === -1 &&
                                !/\*/.test(toRtlFile)
                            ) {
                                stream = gulp
                                    .src(toRtlFile, { allowEmpty: true })
                                    .pipe(rtlcss())
        ...
        function (val, key) {
            if (val.hasOwnProperty("src") && val.hasOwnProperty("dist")) {
                if (["custom", "media", "api", "misc"].indexOf(key) !== -1) {
                        ...
                        if (val.src.styles[0].indexOf("style.scss") !== -1) {
                            presets.forEach((preset) => {
                                let buildStylePresetTask = (cb) => {
                                    val.src.styles[0] =
                                        "{$config.path.src}/sass/presets/" +
                                        preset +
                                        "/style.scss";
                                    val.dist.styles =
                                        "{$config.dist}/css/style." +
                                        preset +
                                        ".bundle.css";
                                    bundle(val);
                                    cb();
                    ...
                    if (
                        typeof val.src.override !== "undefined" &&
                        val.src.override.styles[0].indexOf("plugins.scss") !==
                            -1
                    ) {
                        presets.forEach((preset) => {
                                ...
                                val.src.styles.forEach((file, i) => {
                                    if (file.indexOf("plugins.scss") !== -1) {
                                        val.src.styles[i] =
                                            ...
                                            "/plugins.scss";
                                        val.dist.styles =
                                            "{$config.dist}/plugins/global/plugins." +
                                            preset +
                                            ".bundle.css";
                                        bundle(val);
                                        cb();

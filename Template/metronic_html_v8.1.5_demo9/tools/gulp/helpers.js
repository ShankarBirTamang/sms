...
const bundle = (bundle) => {
    ...
    if (bundle.hasOwnProperty("src") && bundle.hasOwnProperty("dist")) {
        ...
        if ("mandatory" in bundle.src && "optional" in bundle.src) {
            ...
            for (let vendor in vendors) {
                ...
                for (let type in vendorObj) {
                    ...
                    switch (type) {
                        case "fonts":
                            stream = gulp.src(vendorObj[type], {
                                allowEmpty: true,
                            });
            ...
            objectWalkRecursive(bundle.src, (paths, type) => {
                switch (type) {
                    ...
                    case "images":
                        ...
                        if (
                            (!"mandatory") in bundle.src ||
                            (!"optional") in bundle.src
                        ) {
                            src[type] = src[type].concat(paths);
                        }
                        break;
        ...
        for (let type in bundle.src) {
            ...
            // skip if not array
            if (
                Object.prototype.toString.call(bundle.src[type]) !==
                "[object Array]"
            ) {
                continue;
            }
            ...
            switch (type) {
                        ...
                        if (
                            ...
                            bundle.src[type].forEach((path) => {
                                ...
                                if (
                                    ...
                                    build.config.compile.rtl.skip.indexOf(
                                        baseName(path),
                                    ) === -1
                                ) {
                            ...
                            let shouldRtl = false;
                            if (
                                baseName(bundle.dist[type]) === "style.bundle"
                            ) {
                                shouldRtl = true;
                            }
                ...
                case "images":
                    if (bundle.dist.hasOwnProperty(type)) {
                        stream = gulp.src(bundle.src[type], {
                            allowEmpty: true,
                        });
                        const outputImages = outputChannel(
                            bundle.dist[type],
                        ...
                        bundle.src[type].forEach((file) => {
                            ...
                            if (needBundleFiles.length > 0) {
                                ...
                                for (let g in needBundleByGroup) {
                                    ...
                                    // remove ${config.dist}
                                    const separatorDir = bundle.dist[
                                        type
                                    ].replace(/{\$config\.dist}/, "");
                                    // get sub dir
                                    const needBundleDir =
                                        files[0].split(separatorDir)[1];
                                    const needBundleOutputPath =
                                        needBundleDir.replace(
                                            /^(.*?)\/(bundle)\/.*?\.(js)$/,
                                            "$1.$2.$3",
                                        );
